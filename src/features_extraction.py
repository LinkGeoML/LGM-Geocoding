...
def main():
    """
    Implements the first step of the experiment pipeline. Creates features \
    ...
    copyfile('./config.py', exp_path + '/config.py')

    # Load dataset into dataframe
    df = feat_ut.load_points_df(args['fpath'])
    # Shuffle dataframe
    df = df.sample(frac=1).reset_index(drop=True)
    # Encode labels
    df, encoder = feat_ut.encode_labels(df)

    df.to_csv(results_path + '/train_df.csv', index=False)
    ...
    for train_idxs, test_idxs in skf.split(addresses, targets):
        ...
        os.makedirs(fold_path + '/features')
        os.makedirs(fold_path + '/pickled_objects')

        X_train = feat_ut.create_train_features(df.iloc[train_idxs].reset_index(), results_path, fold_path)
        X_test = feat_ut.create_test_features(df.iloc[test_idxs].reset_index(), results_path, fold_path + '/pickled_objects', fold_path)
        y_train, y_test = df['target'][train_idxs], df['target'][test_idxs]
        np.save(fold_path + f'/X_train.npy', X_train)
        np.save(fold_path + f'/X_test.npy', X_test)
        np.save(fold_path + f'/y_train.npy', y_train)
        np.save(fold_path + f'/y_test.npy', y_test)
        fold += 1

    wrtrs.write_feats_space(results_path + '/included_features.csv')
